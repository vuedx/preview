...
const sourceFiles = FS.readdirSync(workflowSource).filter((fileName) => fileName.endsWith('.yaml'));

for (const sourceFile of sourceFiles) {
  console.log(` - ${sourceFile}`);
  const data = Object.fromEntries(
    Object.entries(
      YAML.load(FS.readFileSync(Path.resolve(workflowSource, sourceFile), 'utf-8'))
    ).filter(([key]) => !key.startsWith('.'))
  );

  Object.values(data.jobs).forEach((job) => {
    job.steps = Array.from(job.steps).flat(2);
  });
